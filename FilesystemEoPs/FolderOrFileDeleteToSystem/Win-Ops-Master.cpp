...
bool OpsMaster::TerminateProcessById(DWORD process_id)
{
...
HANDLE OpsMaster::SetTokenDosDevice(std::wstring device_path, HANDLE htoken, bool impersonate)
{
	if (impersonate == true)
		ImpersonateLoggedOnUser(htoken);
	HANDLE ret = OpsMaster::CreateNativeSymlink(L"\\??\\C:", device_path);
	if (impersonate == true)
		RevertToSelf();
	return ret;
}

HANDLE OpsMaster::SetTokenDosDevice(std::string device_path, HANDLE htoken, bool impersonate)
{
...
{
	//please be aware that GenerateRandomStr function might buggy
	f = true;
	std::wstring randomstr = this->GenerateRandomStr(128);
...
std::wstring OpsMaster::GenerateRandomStr(int lenght)
{
	...
	std::wstring grp = L"0123456789_-~@ABCDEFGHIJKLMNOPQRSTUVWXYZ";
	std::wstring ret = L"";

	for (int i = 0; i < lenght; i++) {
		ret += grp[rand() % grp.size()];
	}
	return ret;
}
