// This file adapted from work by Abdelhamid Naceri https://github.com/klinix5/InstallerFileTakeOver
...
HANDLE OpsMaster::OpenFileNative(std::wstring file, DWORD access_mask, DWORD share_mode, DWORD creation_dispostion, DWORD create_options)
{
	...
	case OPEN_EXISTING:
		status = _NtCreateFile(&ret, access_mask, &objattr, &io, NULL, NULL, share_mode, FILE_OPEN,
			FILE_NON_DIRECTORY_FILE | create_options, NULL, NULL);
		break;
	case OPEN_ALWAYS:
...
HANDLE OpsMaster::OpenFileNative(std::string file, DWORD access_mask, DWORD share_mode, DWORD creation_dispostion, DWORD create_options)
{
	return OpsMaster::OpenFileNative(std::wstring(file.begin(), file.end()),
		access_mask, share_mode, creation_dispostion, create_options);
}
...
bool OpsMaster::CreateMountPoint(HANDLE hdir, std::wstring target, std::wstring printname)
{
	...
	DWORD cb = 0;
	return DeviceIoControl(hdir, FSCTL_SET_REPARSE_POINT, rdb, totalsz, nullptr, NULL, &cb, NULL) == TRUE;
}
...
bool OpsMaster::RenameFile(HANDLE hf, ULONG flags, const wchar_t* dest)
{
	IO_STATUS_BLOCK statusBlock;
	size_t bufSizeInBytes = wcslen(dest) * sizeof(dest[0]);
	unsigned int sizeOfRenameInfo = sizeof(FILE_RENAME_INFORMATION) + bufSizeInBytes - sizeof(dest[0]);
	FILE_RENAME_INFORMATION* renameInfo = (FILE_RENAME_INFORMATION*)calloc(1, sizeOfRenameInfo);
	renameInfo->ReplaceIfExists = TRUE;
	renameInfo->RootDirectory = NULL;
	renameInfo->FileNameLength = wcslen(dest) * sizeof(dest[0]);
	memcpy_s(renameInfo->FileName, bufSizeInBytes, dest, bufSizeInBytes);
	NTSTATUS status = _NtSetInformationFile(hf, &statusBlock, renameInfo, sizeOfRenameInfo, FileRenameInformation);
	free(renameInfo); 
	if (status != STATUS_SUCCESS) {
		SetLastErr(_RtlNtStatusToDosError(status));
		return false;
