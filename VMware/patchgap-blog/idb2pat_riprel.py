# FLARE IDA Pro Script Series: Generating FLAIR function patterns using IDAPython by WILLI BALLENTHIN
# https://www.mandiant.com/resources/flare-ida-pro-script
...
class Config(object):
    def __init__(self, min_func_length=6, pointer_size=4, displace_size=4, mode=ALL_FUNCTIONS, pat_append=False, logfile="", loglevel="DEBUG", logenabled=False):
    ...
    def update(self, vals):
        ...
        self.displace_size = vals.get("displace_size", self.displace_size)
...
def find_ref_loc(config, ea, ref):
    ...
    if inst.Op1.type == o_near or inst.Op1.type == o_mem or inst.Op2.type == o_mem:
        ref = (ref - get_item_end(ea)) & ((1<<config.displace_size*8)-1)
    else: return BADADDR

    if is_code(get_full_flags(ea)):
        for ref_ea in zrange(ea, max(ea, 1 + get_item_end(ea) - config.displace_size)):
            if get_dword(ref_ea) == ref:	# read and check displacement
                return ref_ea
...
def make_func_sig(config, func):
    ...
    while ea != BADADDR and ea < func.end_ea:
        ...
        if ref != BADADDR:
            ...
            if ref_loc != BADADDR:
                ...
                for i in zrange(config.displace_size):
    ...
    public_format = " :%%0%dX %%s" % (config.displace_size)
    ...
    for ref_loc, ref in refs.iteritems():
        ...
        if ref_loc >= func.start_ea:
            ...
            ref_format = " ^%%0%dX %%s" % (config.displace_size)
        else:
            # this will be either " ^-%04d %s" or " ^-%08d %s"
...
def make_func_sigs(config):
    ...
    if config.mode == USER_SELECT_FUNCTION:
        ...
        if f is None:
            ...
            return []
        jumpto(f.start_ea)
        if not has_any_name(getFlags(f.start_ea)):
        ...
        for f in get_functions():
            if has_name(getFlags(f.start_ea)) and f.flags & FUNC_LIB == 0:
                try:
                    sigs.append(make_func_sig(config, f))
