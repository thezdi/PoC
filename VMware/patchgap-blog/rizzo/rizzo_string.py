...
class Rizzo(object):
    ...
    def load(self):
        ...
        fp=open(self.sigfile,"rb").readlines()
        fp=map(lambda x:x.replace(b"\r\n","\n"),fp)
        with open(self.sigfile,"wb") as j:
            for i in fp:
                j.write(i)
                
        sigs = pickle.load(open(self.sigfile,"rb"))
        #fp.close()
        print ("done.")
        return sigs
            ...
            if ida_shims.get_name_ea_simple(name) == idc.BADADDR:
                if ida_shims.set_name(ea, name):
                    ida_shims.set_func_flags(
                        ea, (idc.get_func_flags(ea) | idc.FUNC_LIB))
                    return 1
        return 0
